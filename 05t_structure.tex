...
ミューテーションの定義で現れる評価射を説明する．三角圏 $\mathcal D$ と $E,F\in\mathcal D$ に対して，
\[
\RHom(E,-)
:
\mathcal D
...
\bigr)
\]
が存在する．特に，$V=\RHom(E,F)$とすると，
\[
\mathrm{id}_{\RHom(E,F)}
\in
\Hom_{D^b(\mod k)}
\bigl(
\RHom(E,F),
...
\begin{lemm}
任意の $X,Y\in\mathcal D$ および $V\in D^b(\mod k)$に対して同型
\[
\RHom(X,V\otimes Y)
\simeq
V\otimes\RHom(X,Y).
...
\end{lemm}
\begin{proof}
$D^b(\mod k)$ は半単純であるから，任意の$V\in D^b(\mod k)$ に対して
\[
V\simeq \bigoplus_i H^i(V)[-i]
\]
が成り立つ．したがって
\[
V\otimes Y
\simeq
\bigoplus_i H^i(V)\otimes Y[-i]
\]
である．これに $\RHom(X,-)$ を作用させると
\[
\begin{aligned}
\RHom(X,V\otimes Y)
&\simeq
\RHom\left(X,\bigoplus_i H^i(V)\otimes Y[-i]\right)\\
&\simeq
\bigoplus_i H^i(V)\otimes \RHom(X,Y[-i])\\
&\simeq
\bigoplus_i H^i(V)\otimes \RHom(X,Y)[-i]\\
&\simeq
V\otimes \RHom(X,Y).
...
\begin{defn}
$\mathcal D$ を三角圏とし，$E,F\in\mathcal D$ を対象とする． $F$ による $E$ の左ミューテーション $L_FE$ を，完全三角形：
\[ E \xrightarrow{\mathrm{coev}} \RHom(E,F)^\vee\otimes F \longrightarrow R_FE \longrightarrow E[1],\]
によって定義する．同様に，$E$ による $F$ の右ミューテーション $R_EF$ を，完全三角形：
...
(E_1,\dots,E_n).
\]
をその充満例外対象列とする．このとき，各 $1\le i<n$ に対して定まる左ミューテーション：
\[
...
\begin{proof}
$(E,F)$ を例外対象列とする．すなわち$\RHom(F,E)=0$である．左ミューテーション $L_EF$ は，完全三角形：
...
(L_EF,E).
\]
は例外対象列である．最後に充満性に関しては，
\[
F\in \langle L_EF,E\rangle .
\]
から明らか．ゆえに，充満例外対象列
\[
(E_1,\dots,E_n)
