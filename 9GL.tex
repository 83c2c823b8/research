...
\begin{prop}
	$\chi_A = 0$である$D^b(\qgr R_{A,\Lambda})$は，分数的Calabi-Yau圏である．すなわちSerre函手$\Serre$に対して，$\Serre^n \simeq [m]$となるような$m,n\in\ZZ$が存在する．
\end{prop}
\begin{proof}
$\vec{\omega} = \vec{c} - \vec{x_1} - \vec{x_2} - \vec{x_3}$
\begin{itemize}
	\item $(2,3,6)$の場合\\
		$\vec{c} = 2\vec{x_1} = 3\vec{x_2} = 6\vec{x_3}$\\
		$6\vec{\omega} = 6\vec{x_1} +6\vec{x_1} +  6\vec{x_2}  - 6(\vec{x_1}+ \vec{x_2} + \vec{x_3}) = 0$
	\item $(2,4,4)$の場合\\
		$\vec{c} = 2\vec{x_1} = 4\vec{x_2} = 4\vec{x_3}$\\
		$4\vec{\omega} = 2\vec{x_1} +2\vec{x_1} +  4\vec{x_2} + 4\vec{x_3} - 4(\vec{x_1}+ \vec{x_2} + \vec{x_3}) = 0$
	\item $(3,3,3)$の場合\\
		$\vec{c} = 3\vec{x_1} = 3\vec{x_2} = 3\vec{x_3}$\\
		$3\vec{\omega} = 3\vec{x_1} + 3\vec{x_2} + 3\vec{x_3} - 3(\vec{x_1}+ \vec{x_2} + \vec{x_3}) = 0$
	\item $(2,2,2,2)$の場合\\
		$\vec{\omega} = 2\vec{c} - \vec{x_1} - \vec{x_2} - \vec{x_3} - \vec{x_4}$\\
		$\vec{c} = 2\vec{x_1} = 2\vec{x_2} = 2\vec{x_3}$\\
		$2\vec{\omega} = 2\vec{x_1} + 2\vec{x_2} + 2\vec{x_3} +2\vec{x_4} - 2(\vec{x_1}+ \vec{x_2} + \vec{x_3} + \vec{x_4}) = 0$
\end{itemize}
$\Serre = (-\otimes \mathcal{O}(\vec{\omega}))[1]$なので，$\Serre^m = [m]$であることが確かめられる．
\end{proof}

\begin{thm}\cite{MMS09}
...
\begin{prop}
	...
	\begin{align*}
		...
		\end{cases}
	\end{align*}
	したがって，自己同型群の部分群$G$としてあり得るものは，$G = \ZZ/2\ZZ,\ \ZZ/3\ZZ,\ \ZZ/4\ZZ,\ \ZZ/6\ZZ $である．
\end{prop}
...
\begin{prop}
	$C$を楕円曲線とすると，$\Stab(C)$は有限群の作用で不変である．したがって，任意の群$G$に対して$\Gamma_X = \Stab(C)$となり，
	\[\Forg_G^{-1}\colon  \Stab(D^b(\Coh C))\hookrightarrow \Stab(D^b_{G}(\Coh C)) \simeq \Stab(D^b(\Coh \left[X/G\right])).\]
と埋め込むことができる．
\end{prop}

\begin{prop}
	$Q$をDynkin箙とする．このとき，$D^b(\rep_k(Q))\simeq D^b(\mod kQ)$は分数的Calabi-Yau圏である．
\end{prop}
...
\begin{proof}
	分数的Calabi-Yau
	\begin{align}
		\xi^n\cdot\sigma = \Serre^n\cdot \sigma = [m]\cdot \sigma = m\cdot \sigma
	\end{align}
\end{proof}
