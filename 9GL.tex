...
\begin{thm}
	重み付き射影直線 \(\PP_{A,\Lambda}\) 上の導来圏 \(D^b(\Coh\PP_{A,\Lambda})\) 上のSerre函手を$S$とする．このとき，以下は同値である：\vspace{-3mm}
	\begin{itemize}
		\item $(S,\xi)$に関して$\sigma$がGepner型であるような安定性条件$\sigma$が存在する．
		...
		\[
			\begin{array}{ccccc}
				...
				X & \longmapsto & (\rk(X),\deg(X))& .
					\end{array}
\]
と定めると，$K(\PP_{A,\Lambda})$．Serre函手の擬逆函手$S^{-1}$が$\ZZ^{\oplus 2}$に誘導する線型写像$[S^{-1}]$は，
\begin{align*}
	\begin{pmatrix}
		-1 & 0\\ -a \cdot \chi_{A}& -1
	\end{pmatrix},
