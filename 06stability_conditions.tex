...
\begin{prop}
	$\P$を三角圏$\D$のスライスとする．このとき，任意の区間$I\subset\RR$で長さが１未満のとき，その充満部分圏$\P(I)\subset\D$は準アーベル圏になる．
\end{prop}

\begin{prop}\cite{Bri07}
	$\sigma = (Z,\P)$を三角圏$\D$上の安定性条件とする．このとき，$\P(\phi)\subset\D$はアーベル圏となる．
\end{prop}
\begin{proof}
$\P(\phi)$は，アーベル圏$\A=\P((\phi-1,\phi])$の充満部分圏である．したがって，$\Phi(\phi)$での$f$を$\A$と考えたときにその核，余核が$\P(\phi)$に入っていることを言えばよい．
\end{proof}

\begin{defn}
	$X\in\P(\phi)\subset\D$に対して，$X$が$\P(\phi)$において単純対象であるとき$X$は$\P$に関して安定であると定める．
\end{defn}
%\begin{defn}\cite{Bri07}
