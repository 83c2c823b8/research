...
\begin{itemize}
  \item 正の整数からなる列 $A = (a_1, a_2, \dots, a_r)$，
  \item $\mathbb{P}^1(k)$ の互いに異なる点の列 $\Lambda = (\lambda_1, \lambda_2, \dots, \lambda_r)$，
\end{itemize}

ただし，通常 $\lambda_1 = \infty,\ \lambda_2 = 0,\ \lambda_3 = 1$ と正規化する．次に，アーベル群
\[
...
\[
	S = k[X_1, X_2, \dots, X_r], \quad \deg X_i = \vec{x}_i. 
\]
において，以下の $L_A$-斉次イデアル:
\[
	I_{A,\Lambda} = \left( X_i^{a_i} - X_2^{a_2} + \lambda_i X_1^{a_1} \ \middle|\ i = 3, \dots, r \right).
\]
を用いて商環
\[
	R_{A,\Lambda} = k[X_1, X_2, \dots, X_r]\Big{/}I_{A,\Lambda} \quad \deg(X_i)=\vec{x_i}.
\]
を定義する．このとき，$R_{A,\Lambda}$は，部分環$k[X_0^{a_0},X_1^{a_1}]$が存在する．
\end{defn}

$L_A$は階数$1$のアーベル群であるため，各次数$\vec{\ell}\in L_A$は一意的に，
\[\vec{\ell} = \sum_{i=1}^r \ell_i\vec{x_i} + \ell\vec{c}\quad (0\le \ell_i < p_i,\ \ \ell \in \ZZ).\]
と表すことができ，$\ell,m\in L_A$に対して順序$\le$を
\begin{gather*}
\vec{\ell} = \sum_{i=1}^r \ell_i\vec{x_i} + \ell\vec{c}\quad (0\le \ell_i < p_i,\ \ \ell \in \ZZ).\\
\vec{m} = \sum_{i=1}^r m_i\vec{x_i} + m\vec{c}\quad (0\le m_i < p_i,\ \ \ell \in \ZZ).
\end{gather*}
とあらわしたときに，すべての$0\le i\le r$で$\ell_i\le m_i$かつ$\ell\le m_i$とさだめる．ここで，
\[\vec{\omega} \coloneq (r-2)\vec{c} - \sum_{i=1}^r\vec{x_i}.\]
...
\begin{itemize}
  ...
  \item $\vec{\omega} = (r - 2)\vec{c} - \sum_{i=1}^r \vec{x}_i$ は dualizing element（双対化元）である．
\end{itemize}
\end{thm}
\begin{proof}
...
\begin{defn}
	重み付き射影直線 $\PP_{A,\Lambda}$ のパラメータ $A = (a_1, \dots, a_r)$ と $\lambda = (\lambda_1, \dots, \lambda_r)$ に対して，有理数 $\chi_A$ を以下のように定義する:
\[\chi_A = 2 + \sum_{i=1}^{r} \left( \frac{1}{a_i} - 1\right) .\]
...
\[\rk(M) = \dim_{\CC} \F_{\vec{0}} - \dim_{\CC} \F_{\vec{c}}.\]
 で定義し，群準同型$\delta\colon L_A\to \ZZ$を生成元$\vec{x_i}\quad (1\le i \le r)$に対して
\[\delta(\vec{x_i}) = \frac{a}{a_i}\quad (1\le i\le r).\]
で定義する．ただし，$a\coloneq \text{lcm}(a_1,\ldots ,a_r)$である．
\end{defn}

\begin{rem}
	\hfill\vspace{-3mm}
	\begin{itemize}
		\item $\chi_A > 0$となるのは，$A = (1,a,b),\ (2,2,c),\ (2,3,3),\ (2,3,4),\ (2,3,5)\quad (a,b,c\in\ZZ_{\ge 1})$
		\item $\chi_A = 0$となるのは，$A = (3,3,3),\ (2,4,4),\ (2,2,2,2),\ (2,3,6)$
	\end{itemize}
\end{rem}

\begin{prop}
	次数とよばれる以下の性質をみたす線型写像$\deg\colon K(\Coh(\PP_{A,\Lambda}))\to \ZZ$が一意に存在する．
	\begin{itemize}
		\item[(i)]
			各$\vec{x}\in L_A$に対して，$\deg(\mathcal{O}(\vec{x})) = \delta(\vec{x})$
		\item[(ii)]
			$\deg(\mathcal{O}) = 0$
		\item[(iii)]
			$\mathcal{O}_{\lambda}$が$\lambda$にサポートを持つ単純層であるとき，
		\[ \deg(\mathcal{O}_{\lambda}) = \frac{a}{a(\lambda)}.\]
	\end{itemize}
\end{prop}
\begin{proof}
...
\[\deg(\F) \coloneq \overline{\chi}(\F) - \rk(\F)\overline{\chi}(\mathcal{O}).\]
...
\begin{align*}
	\overline{\chi}(\mathcal{O}) + \overline{\chi}(\mathcal{O}(a\vec{\omega})) &= \sum_{j=0}^{a-1}\chi(\mathcal{O}(-j\vec{\omega})) +\sum_{j=0}^{a-1}\chi(\mathcal{O}(a-j\vec{\omega}))\\
																																						 ...
																																						 &= 0.
\end{align*}
したがって，$2\overline{\chi}(\mathcal{O}) + a\delta(\vec{\omega}) = 0$である．また，$\vec{\omega} = (r-2)\vec{c} - \sum_{i=0}^r\vec{x_i}$なので，
\begin{align*}
	\delta(\vec{\omega}) &= -\sum_{i=0}^r\frac{a}{a_i} +(r-2)a\\
											 &= -a\qty(\sum_{i=0}^r\frac{1}{a_i} - (r-2))\\
											 &= -a\qty(2 + \sum_{i=1}^r\qty(\frac{1}{a_i} - 1)))\\
											 &= -a\cdot\chi_{A}
\end{align*}
 から示された．
\end{proof}
...
\begin{center}
  \begin{tikzpicture}[->,thick]
      \node (0) at (0,0) {0};
			\node (A1) at (2,2) {$\vec{x_1}$};
			\node (A2) at (5,2) {$2\vec{x_1}$};
      \node (A3) at (8,2) {$\cdots$};
			\node (A4) at (11,2) {$(a_1-1)\vec{x_1}$};
      \node (B1) at (2,1) {$\vec{x_2}$};
      \node (B2) at (5,1) {$2\vec{x_2}$};
      \node (B3) at (8,1) {$\cdots$};
			\node (B4) at (11,1) {$(a_2-1)\vec{x_2}$};
      \node (C1) at (2,0) {$\vdots$};
      \node (C2) at (5,0) {$\vdots$};
      \node (C3) at (8,0) {$\vdots$};
      ...
      \node (D3) at (8,-1) {$\cdots$};
			...
			\node (c) at (13,0) {$\vec{c}$};
      \draw (0) -- node[above] {$X_1$} (A1);
      \draw (A1) -- node[above] {$X_1$} (A2);
      \draw (A2) -- node[above] {$X_1$} (A3);
      \draw (A3) -- node[above] {$X_1$} (A4);
      \draw (A4) -- node[above] {$X_1$} (c);
      \draw (0) -- node[below] {$X_2$} (B1);
      \draw (B1) -- node[above] {$X_2$} (B2);
      \draw (B2) -- node[above] {$X_2$} (B3);
      \draw (B3) -- node[above] {$X_2$} (B4);
      \draw (B4) -- node[pos=.5,fill=white] {$X_2$} (c);
  ...
  \end{tikzpicture}
  \end{center}
	\[X_i^{a_i} = X_1^{a_1} - \lambda_iX_2^{a_2}.\]
	
\end{prop}

\begin{defn}
	加法圏$\C$が以下の条件を満たすときKrull-Schmidt圏という．\\
			任意の$X\in\C$に対して，
			\[X \simeq X_1\oplus \cdots \oplus X_n.\]
			となるような$X_1,\ldots, X_n\in\C$が存在する．ただし，$\End(X_i)\ (1\le i\le n)$は局所環である．
\end{defn}

\begin{prop}
	$\C$を加法圏，$X\in\C$とする．このとき，(1) $\Longrightarrow$ (2) $\Longrightarrow$ (3)が成り立つ．\vspace{-3mm}
	\begin{itemize}
		\item[(1)]
			$\End(X)$が局所環である．
		\item[(2)]
			$\End(X)$が$0,1$以外に冪等元を持たない．
		\item[(3)]
			$X$は既約である．
	\end{itemize}
\end{prop}
\begin{proof}
	(1) $\Longrightarrow$ (2)\\
	$e\in\End(X)$を冪等元，すなわち$e^2 = e$とする．$e + (1-e) = 1$なので，唯一の極大イデアルである非単元全体の集合に$e,1-e$の両方が入ることはない．したがって，$e$か$1-e$が単元である．$e$が単元のとき，
	\[e = e^2\cdot e^{-1} = e\cdot e^{-1} =1.\]
	$1-e$が単元のとき，
	\[1-e = (1-e)^2\cdot (1-e)^{-1} = (1-e)\cdot (1-e)^{-1} =1.\]
	したがって，$e = 0,1$がわかった．\\
	(2) $\Longrightarrow$ (3)\\
	対偶を示す．$X$が既約でないとすると，$X\simeq Y\oplus Z\ (Y,Z\neq 0)$となるが，
	$\begin{pmatrix}\id_Y& 0\\0&0 \end{pmatrix}\in\End(X)$が非自明な冪等元となっている．
\end{proof}
$\End(X)$が有限次元$k$-代数となっているときは，$(1)\Longleftarrow (2)$もなりたつ．

\begin{thm}[Krull--Schmidt 性]\cite{GL87}
	重み付き射影直線 $\PP_{A,\Lambda} $ に対して，連接層のアーベル圏
\[
	\Coh(\PP_{A,\Lambda}) = \qgr R_{A,\Lambda} = \gr R_{A,\Lambda}\Big{/}\tors R_{A,\Lambda}.
\]
は Krull--Schmidt 圏である．すなわち，任意の対象 $\mathcal{F} \in \Coh(\PP_{A,\Lambda})$ は直既約対象の有限直和に分解でき，その分解は同型と順序を除いて一意である：
\[
...
\begin{center}
  \begin{tikzpicture}[->,thick]
      \node (0) at (0,0) {$\mathcal{O}{(\vec{0}})$};
			\node (x1) at (1.5,1.5) {$\mathcal{O}{(\vec{x_1}})$};
			\node (c) at (2,0) {$\mathcal{O}{(\vec{c}})$};
      \draw (0) -- node[left] {$x_1$} (x1);
      \draw (x1) -- node[right] {$x_1$} (c);
      \draw (0) -- node[below] {$x_2$} (c);
  \end{tikzpicture}
  \end{center}
	\[\vec{c} = 2\vec{x_1} = \vec{x_2}.\]
このとき，次の列
\[
	(\mathcal{O},\ \mathcal{O}(\vec{x_1}),\ \mathcal{O}(\vec{c})),
\]
