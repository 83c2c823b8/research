...
\begin{defn}
加法圏 $\A$ において任意の射が核と余核をもつとき，$\A$ をプレアーベル圏(pre-abelian category)という．$\A$ をプレアーベル圏とし，このとき標準的な分解：
\[
...
\]
が同型であるとき，$f$ はstrict であるという．

プレアーベル圏 $\A$ において次の二条件が成り立つとき，
$\A$ を準アーベル圏(quasi-abelian category)という．
\begin{enumerate}
    \item 任意の strict monomorphism の pushout が
    strict monomorphism である．
    \item 任意の strict epimorphism の pullback が
    strict epimorphism である．
\end{enumerate}
\end{defn}
...
X' \xrightarrow{f'} Y' \xrightarrow{g'} Z' \xrightarrow{h'} X'[1].
\]
が与えられているとする．このとき，射の三つ組 $(\alpha, \beta, \gamma)$（ただし $\alpha \colon X \to X'$，$\beta \colon Y \to Y'$，$\gamma \colon Z \to Z'$）が以下の図式が可換にするとき，完全三角形の間の射（morphism of triangles）であるという：
\[
\begin{tikzcd}
